...
**Anonymous is the default.** Never wire a real user identifier into Pubky Pulse on your own initiative. This covers \`Pulse.setUser()\` (Web, Swift, Android) and \`Pulse.withUser()\` and per-request user scoping (Node). \`Pulse.setUserProperties()\` is not gated separately — it attaches to whichever id is already in play, so it follows the answer. \`Pulse.clearUser()\` is never gated — it removes a link rather than creating one, so wire it on logout, on a shared device, and when moving an app off identified analytics; on Swift and Android the stored identifier outlives the process until it runs.
