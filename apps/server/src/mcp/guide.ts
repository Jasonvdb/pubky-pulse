...
- **Notifications**: two types fire on production issues. \`issue.new\` is an inbox summary that fires from \`issue_scan\` at the end of every hourly run when anything was just created or regressed (one alert per team per scan); its email delivery is off by default. \`issue.digest\` is the per-project inbox digest gated by \`issue_alert_frequency\` (none/hourly/6-hourly/daily/weekly); its email delivery is on by default. See the Notifications section for delivery details.
...
**Browser admission:** the hardened Web SDK accepts files up to 5 MiB, with 20 files / 20 MiB of active and pending payload per loaded module. This intentionally replaces its earlier 2 GiB file limit; server quotas/wire formats and the other SDK limits are unchanged. Excess files are dropped before reading or hashing. Attachment flush waits stop after 120 seconds, while noncancelable work keeps its allocation reservation until it settles. Check the [Web SDK attachment limits](https://github.com/Jasonvdb/pubky-pulse-web#attachments) before asking for a browser reproduction.
...
### Notifications
Pubky Pulse has a unified, multi-channel notification system: each user-facing event (new feedback, new/regressed issues, manual job completion) always writes a row to each eligible user's inbox \`notifications\` table, then records whichever delivery channels are enabled — \`in_app\` and email (Resend). New channels (Telegram, Slack, etc.) plug in as new \`ChannelAdapter\`s without producer changes. Per-user channel preferences remain in the shared/backend contract under \`users.preferences.notifications.types\` and are merged into \`PATCH /v1/auth/me\`, including \`in_app\`; the dashboard preference page exposes only Email, and inbox rows do not depend on either channel preference. Sign-in verification codes stay transactional (sent directly via EmailService) because their recipient may not yet be a user; there is no invite flow to email, since access is granted by configured email domain. **Notifications are user-scoped, not team-scoped, so they do not have MCP tools** — humans read them in the web dashboard (\`/dashboard/notifications\`).

**Issue notification types** — there are two:
- \`issue.new\` fires from \`issue_scan\` at the end of every hourly run, with one alert per team summarizing all production issues that were just created or regressed. Every eligible recipient gets a dashboard inbox row; email is off by default. It bypasses any cadence throttle, so the alert lands within ~5 min of detection.
- \`issue.digest\` fires from \`issue_notify\` at the project's \`issue_alert_frequency\` (none/hourly/6-hourly/daily/weekly). When produced, every eligible team member gets a dashboard inbox row; email is on by default. Setting the project frequency to \`none\` prevents digest production entirely, while each user's dashboard Email preference controls only whether a produced digest is also emailed.
...
**Anonymous is the default.** Never wire a real user identifier into Pubky Pulse on your own initiative. This covers \`Pulse.setUser()\` (Web, Swift, Android) and \`Pulse.withUser()\` and per-request user scoping (Node). \`Pulse.setUserProperties()\` is not gated separately — it attaches to whichever id is already in play, so it follows the answer. \`Pulse.clearUser()\` is never gated — it never creates a link, so you may always write it; on Swift and Android the stored identifier outlives the process until it runs. It is not an unlink: a bare \`clearUser()\` restores the previous anonymous id, which ingest re-resolves to the account that already claimed it, so future activity stays attributed. Separating future activity takes a fresh anonymous id — \`newAnonymousId: true\` (Kotlin \`newAnonymousId = true\`). Neither form undoes the server-side claim on events already attributed; \`clearUser\` is not a privacy or data-deletion control.
...
- **Web** ([github.com/Jasonvdb/pubky-pulse-web](https://github.com/Jasonvdb/pubky-pulse-web)) — framework-agnostic browser SDK (\`@synonymdev/pubky-pulse-web\`): package installation, \`Pulse.configure()\` with only the app's client key, event logging with automatic capture of uncaught errors and unhandled rejections, automatic screen tracking on History API navigation, structured metrics, funnels, user identity (\`Pulse.setUser()\` flushes and claims the anonymous history — opt-in only, see **Identity Is Opt-In**), user properties, **error attachments**, **feedback** (\`Pulse.sendFeedback\`), **questionnaires** (\`Pulse.fetchQuestionnaire\` / \`saveQuestionnaireResponse\` plus pure answer helpers — the SDK ships no UI), and \`propagateSessionTo\` to forward \`X-Pulse-Session-Id\` to your own API for session correlation with the Node SDK. Create the app with \`platform: "web"\`, the site's origins in its \`allowed_origins\` — including the dev server's (\`http://localhost:5173\`). Ingest refuses a browser request whose \`Origin\` is not on that list, and CORS allows the ones that are; nothing needs to change in the server's \`CORS_ORIGINS\`, which is the dashboard's own origin. Set \`appVersion\` in \`Pulse.configure()\` too: without it issue regression detection and the latest-version badges have nothing to compare.
...
For browser integrations, prefer \`Pulse.init()\` for optional setup and inspect its diagnostic result. Telemetry collectors contain their own failures, while strict \`configure()\`, invalid identity input, feedback and questionnaire requests retain explicit error contracts. Handle those calls at the application boundary. Fetch propagation and metadata are best effort: do not require the session header or every network attribute. The Web SDK also applies local event/queue budgets (128 KiB/event after hooks, 4 MiB buffer, 1 MiB offline budget); concurrent unload writes make the cross-tab aggregate cap best effort. See the [Web SDK README](https://github.com/Jasonvdb/pubky-pulse-web#host-application-boundaries) for the supported behavior and limits.

### Agent skills

The SDK bullets above say what each SDK covers; the step-by-step procedures live in the agent skills repo [github.com/Jasonvdb/pubky-pulse-skills](https://github.com/Jasonvdb/pubky-pulse-skills). \`pubky-pulse-instrument\` audits a codebase for its web, backend, iOS and Android surfaces, drafts a tracking plan, creates the project and apps with these tools, wires each SDK with full error coverage, and verifies data arrives. \`pubky-pulse-web\`, \`-node\`, \`-swift\` and \`-android\` cover one SDK each, \`pubky-pulse-investigate-issues\` triages issues, and \`pubky-pulse-operations\` covers queries and admin. If those skills are installed in your harness, use them; if not, suggest installing them (Claude Code: \`claude plugin marketplace add Jasonvdb/pubky-pulse-skills\` then \`claude plugin install pubky-pulse-skills@pubky-pulse-skills\`; other agents follow the repo README).

## Key Notes

- User identity is opt-in: never write \`setUser\`/\`withUser\` without the developer's explicit yes. See **Identity Is Opt-In**.
- \`bundle_id\` is **immutable after creation** — to change it, an owner must delete and recreate the app from the dashboard; there is no \`delete-app\` MCP tool. Web/backend apps need no bundle_id; legacy values remain metadata. A web app's \`allowed_origins\`, by contrast, is editable with \`update-app\` at any time.
