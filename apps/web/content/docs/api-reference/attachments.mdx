...
{
  "attachment_id": "uuid",
  "upload_url": "https://api.yourdomain.com/v1/ingest/attachment/<id>",
...
```json
{
  ...
  "created_at": "2024-01-15T10:30:00.000Z",
  "download_url": {
    "url": "https://api.yourdomain.com/v1/attachments/download?t=...",
...
Returns the file bytes with `Content-Type`, `Content-Length`, `Content-Disposition: attachment`, `X-Content-Type-Options: nosniff`, and `Cache-Control: private, no-store`. If the server is configured with `PULSE_ATTACHMENTS_INTERNAL_URI`, the response uses `X-Accel-Redirect` so nginx streams bytes directly without passing them through Node.
...
**Auth required:** Yes (JWT only — agent, client and import keys receive `403`) **and ownership of the attachment's project.**
...
Deleting an attachment is human-only: an agent key is refused even when its creator owns the project. Returns 404 if the attachment is missing, already soft-deleted, or belongs to a project outside the caller's team; 403 if the caller can see it but does not own its project.
