...
An event from a browser fills the same fields with browser values — `device_model` is the browser, `os_version` the OS name and version, `screen_name` the URL path, and `build_number` is `null`:
