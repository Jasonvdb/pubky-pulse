...
| `environment` | string | Filter by environment: `ios`, `ipados`, `macos`, `watchos`, `android`, `web`, `backend`. |
| `screen_name` | string | Filter by screen name. Matches the value exactly, or any path nested beneath it — `/checkout` also returns `/checkout/payment`, but not `/checkout-abandoned`. |
| `device_model` | string | Filter by device model (exact match). On web this is the browser and its major version, e.g. `Chrome 120`. |
| `os_version` | string | Filter by OS version (exact match). On web this is the OS name and version, e.g. `macOS 10.15.7`. |
...
An event from a browser fills the same fields with browser values — `device_model` is the browser, `os_version` the OS name and version, `screen_name` the recorded screen name (URL path by default), and `build_number` is `null`:
...
See [Field semantics by platform](/docs/concepts/events#field-semantics-by-platform).
...
Accepts the same filter parameters as `GET /v1/events` (except `cursor`, `limit`, and `order`): `team_id`, `project_id`, `app_id`, `level`, `user_id`, `session_id`, `environment`, `screen_name`, `device_model`, `os_version`, `since`, `until`, `data_mode`. `screen_name` matches by path prefix here too.
