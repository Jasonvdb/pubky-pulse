...
List funnel definitions across every accessible project in a team. Powers the dashboard's "All projects" view. Each row carries `project_id` so callers can group by project.
...
|-----------|------|-------------|
| `team_id` | string | Required. Restricts the listing to the given team. The caller must have access; otherwise the response is `{ "funnels": [] }`. |
...
**Auth required:** Yes (`funnels:write` permission or JWT) **and ownership of the project.** A viewer receives `403`.
...
**Auth required:** Yes (`funnels:write` permission or JWT) **and ownership of the project.** A viewer receives `403`. Deleting a funnel definition is one of the destructive operations agent keys *are* allowed to perform.
...
Aggregate counts of funnel starts and completions across all active funnels the caller can access, counted as unique users (open-mode semantics matching `/v1/projects/:id/funnels/:slug/query`). Per funnel: `count` is the distinct users who hit the last step; `started` is the distinct users who hit the first step or the last step (open-mode backfill — reaching a later step implies passing through the first). Guarantees `count ≤ started`. Per-funnel results are summed across all funnels the caller can access. Used by the all-projects dashboard stat HUD.
...
| Field | Type | Description |
|-------|------|-------------|
| `count` | number | Distinct users who completed the funnel (last-step matches), summed across funnels. |
| `started` | number | Distinct users who started or completed the funnel (first-step or last-step matches, open-mode backfill), summed across funnels. |
