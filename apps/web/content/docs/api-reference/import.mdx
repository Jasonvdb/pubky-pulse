...
Historical browser data uses the same shape with the values a browser reports — the browser in `device_model`, the OS name and version in `os_version`, the URL path in `screen_name`, and no `build_number`:
