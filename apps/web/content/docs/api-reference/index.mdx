...
Complete REST API reference for Pubky Pulse.
...
| MCP | `POST /mcp`, `GET /mcp`, `DELETE /mcp` — Model Context Protocol endpoint for AI coding agents. Authenticated with a `pulse_agent_*` Bearer token. See the [MCP docs](/docs/mcp). |
