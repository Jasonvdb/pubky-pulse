...
List metric definitions across every accessible project in a team. Powers the dashboard's "All projects" view. Each row carries `project_id` so callers can group by project.
...
|-----------|------|-------------|
| `team_id` | string | Required. Restricts the listing to the given team. The caller must have access; otherwise the response is `{ "metrics": [] }`. |

Response shape is identical to the project-scoped list endpoint above.
...
**Auth required:** Yes (`metrics:write` permission or JWT) **and ownership of the project.** A viewer receives `403`.
...
**Auth required:** Yes (`metrics:write` permission or JWT) **and ownership of the project.** A viewer receives `403`. Deleting a metric definition is one of the destructive operations agent keys *are* allowed to perform.
...
## GET /v1/metrics/completions/count

Aggregate count of metric completions (events with `phase = "complete"`) and failures (events with `phase = "fail"`) across all metrics the caller can access. Used by the all-projects dashboard stat HUD to render success/total + success rate.
...
`count` is complete-phase events; `failed` is fail-phase events. Cancels and `record`-phase events are excluded. Compute success rate as `count / (count + failed)`.

## GET /v1/projects/:projectId/metric-stats

Per-metric success/fail counts for every metric definition in a project. One row per metric, including metrics with zero events (returned as `0/0`). Used by the metrics list cards.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `since` | string | Start time. Relative or ISO 8601. Defaults to 24 hours ago. |
| `until` | string | End time. Relative or ISO 8601. |
| `data_mode` | string | `production` (default), `development`, or `all`. |
...
`success_rate` is `complete_count / (complete_count + fail_count) * 100`, rounded to two decimals, or `null` when both counts are zero.

## GET /v1/metric-stats

Per-metric success/fail counts across every metric definition in every accessible project in a team. Powers the dashboard's "All projects" metrics view. Rows are grouped by `(project_id, slug)` because slugs are only unique within a project.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `team_id` | string | Required. Restricts the rollup to the given team. The caller must have access; otherwise the response is `{ "stats": [] }`. |
| `since` | string | Start time. Relative or ISO 8601. Defaults to 24 hours ago. |
| `until` | string | End time. Relative or ISO 8601. |
| `data_mode` | string | `production` (default), `development`, or `all`. |
...
Same `success_rate` formula as the project-scoped variant. The extra `project_id` lets callers disambiguate same-slug metrics across projects.
