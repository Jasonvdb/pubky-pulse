...
- **Revenue** is filtered server-side by `app_users.first_seen_at >= now() - 360 days`, so users acquired before the spend window's start don't contribute revenue against zero matchable spend.
...
Manual triggers fire **both** jobs in parallel (`POST /v1/projects/:id/ads/sync` or the `sync-ads` MCP tool) — useful right after onboarding a new RevenueCat or Apple Search Ads integration, or to force-refresh without waiting for the cron. Admin-only.
...
| `status` | Network-side status snapshot — `ENABLED` / `PAUSED` / `ON_HOLD` / `DELETED` for Apple Search Ads. Surfaces with a tone-coded badge in the dashboard |
...
ROAS is tone-coded the same way across web and iOS via shared helpers (`roasTone`, `formatRoasLabel` in `@owlmetry/shared`): **green** when ≥ 1.0× (earning back), **amber** when ≥ 0.5× (recovering half), **red** below that, muted "—" when null.

## All projects vs. single project

The dashboard's `/dashboard/ads` page opens in **All projects** mode when the team has 2+ projects — campaigns are ranked across the whole team, with each row tagged by its owning project so you can compare acquisition performance side-by-side without flipping the project picker. Switch to a single project to see its campaigns in isolation. The MCP `list-ad-campaigns` tool mirrors this with an optional `team_id`, mutually exclusive with `project_id`. Drill-downs (ad groups, keywords/ads) stay project-scoped — clicking into a row uses its `project_id`.
...
If users *do* carry the right attribution but revenue is zero across the board, RevenueCat hasn't synced yet — manually trigger via the dashboard "Sync now" button or the `sync-integration` MCP tool.

If you recently shipped the Owl SDK into an app that already has paying users in RevenueCat, run the [`revenuecat_user_backfill`](/docs/concepts/background-jobs) job to import those historical RC customers as `app_users` rows so their lifetime revenue can attribute against the campaigns that originally acquired them — without it, only users seen by the SDK after install (or surfaced via live RC webhooks) appear here.
