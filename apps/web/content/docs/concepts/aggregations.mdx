...
Pubky Pulse maintains pre-computed daily and hourly rollup tables for the four event-shaped data sources (`events`, `metric_events`, `funnel_events`, `questionnaire_responses`). These power the subtle sparkline charts on dashboard cards today and any time-range trend query going forward.
...
The 3-bucket re-aggregation window absorbs late-arriving events from offline SDKs. Events arriving later than that need a manual backfill (see below).

## Backfill

Backfilling the rollup tables is an **operator action** with no public API or MCP surface — by design. The trigger-job route rejects `stats_aggregate_*` system jobs to keep a team-scoped agent key from kicking off system-wide work.
