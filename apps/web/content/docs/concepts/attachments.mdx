...
When an error cannot be reproduced without the original input file — a failed media conversion, a 3D model that won't parse, a PDF that fails to decode — SDKs can upload the file alongside the error event. The attachment is stored on the server, linked to the event, and automatically linked to the resulting issue. Engineers download it from the dashboard or MCP to reproduce locally.
...
Checks happen at reserve time against the client-declared `size_bytes`, before bytes stream over the wire. Reservations that would exceed the per-user bucket return `413 user_quota_exhausted`; ones that would exceed the project ceiling return `413 quota_exhausted`. The event itself still posts — only the attachment is dropped. SDKs treat quota rejection as an attachment failure; the associated event remains eligible for delivery.
...
The [Web SDK browser limits](/docs/sdks/web/attachments#browser-resource-limits) are additional local
safeguards: 5 MiB per file, with bounded queued payload and waiting. This intentionally lowers the
browser's earlier 2 GiB ceiling. It changes no server quota or wire format; Node.js, Swift and Android
retain their existing SDK limits.
...
do {
    ...
    Pulse.error(
        ...
        attachments: [
            ...
            PulseAttachment(data: debugJSON, name: "debug.json",
...
- `delete-attachment` — **human-only**; every agent call returns `403`. Ask a project owner to delete from the dashboard or the REST API.
...
- **Soft delete + grace**: `DELETE /v1/attachments/:id` (or the dashboard) sets `deleted_at`. Deletion requires a human owner of the project the attachment belongs to — see [Authentication & Access Control](/docs/concepts/authentication#human-only-operations). The `attachment_cleanup` daily job hard-deletes rows after 7 days.
...
- Attachment bytes live on disk at `PULSE_ATTACHMENTS_PATH` (default `/opt/pubky-pulse-attachments` in production, `./data/attachments` in dev). **Do not include this path in your `pg_dump` backups.**
- For high-throughput deployments, mount a dedicated volume at this path so disk-full on attachments cannot starve Postgres.
- When `PULSE_ATTACHMENTS_INTERNAL_URI` is set, the server replies with `X-Accel-Redirect` headers so nginx can serve downloads without streaming through Node. Without it, the server streams bytes directly — fine in dev, avoid in production for any file above a few megabytes.
