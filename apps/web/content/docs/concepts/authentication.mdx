...
Pubky Pulse uses passwordless email verification for human users and API keys for programmatic access. All resources are scoped to teams with role-based access control.
...
Users can belong to multiple teams. The dashboard includes a team switcher when you have more than one team; agent keys are scoped to a single team, so an agent working across teams needs one key per team.
...
| SDK (client) | `pulse_client_` API key | `permissions` array on the key |
| MCP client / automation (agent) | `pulse_agent_` API key | `permissions` array on the key |
