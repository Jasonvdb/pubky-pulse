...
API keys are used by SDKs, MCP clients, and automated systems to interact with the Owlmetry API. There are three types:

| Type | Prefix | Used By | Purpose |
|---|---|---|---|
| Client | `owl_client_` | SDKs | Event ingestion from apps |
| Agent | `owl_agent_` | MCP clients, automation | Querying data and managing resources |
...
| MCP client / automation (agent) | `owl_agent_` API key | `permissions` array on the key |
