...
| `revenuecat_user_backfill` | RevenueCat User Backfill | Project | Manual | Pages through every customer in the linked RevenueCat project (V2 list-customers API) and creates or updates the corresponding `app_users` row (subscription state, lifetime USD revenue, attribution). Idempotent — re-running is safe. Anonymous RevenueCat IDs (`$RCAnonymousID:*`) are skipped. Use after installing the Owl SDK in an existing app to retrofit historical RC customers so [Advertising Insights](/docs/concepts/advertising-insights) can attribute their revenue. Can take hours on large projects (~3 RC API calls per customer at the 480 req/min Customer Information budget) |
