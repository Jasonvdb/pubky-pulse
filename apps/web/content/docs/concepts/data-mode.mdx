---
title: Data Mode
description: Filter between production and development data across the dashboard, MCP, and API.
---

Pubky Pulse separates production and development data so you can analyze real user behavior without noise from test builds. Every event, metric event, and funnel event carries an `is_dev` boolean that indicates whether it came from a development build.
...
| Android | `true` when the host app is debuggable (the `FLAG_DEBUGGABLE` manifest flag — true for debug builds, false for release) |
| Web | `true` when the page is served from `localhost`, `127.0.0.1`, or a `file:` URL |
...
MCP tools that query data accept a `data_mode` argument (`production`, `development`, or `all`) — `query-events`, `query-metric`, `query-funnel`, `list-app-users`, and `list-user-locales` among them:

```json
{ "project_id": "<id>", "data_mode": "development" }
```
