...
The step filter is an **exact** match on the recorded screen name: `screen_name: "/checkout"` counts `/checkout` but not `/checkout/payment`. For dynamic web routes such as `/orders/8f21/confirm`, use [`screenNameForPath`](/docs/sdks/web/screens#mapping-route-names) to record a stable screen name and match that value, or name the step explicitly with `Pulse.step()`.
