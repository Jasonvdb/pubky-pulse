...
2. **Add the integration** via the dashboard (Project > Integrations > Connect RevenueCat) or the `add-integration` MCP tool, with `provider: "revenuecat"` and your RevenueCat secret API key.

   A webhook secret is auto-generated. The response (MCP and API alike) includes a `webhook_setup` section with the exact values to paste into RevenueCat.
...
3. **Configure the webhook** in RevenueCat dashboard (Settings → Webhooks → + New Webhook) using the values from `webhook_setup`:
   ...
   Lost the post-create output? Re-reveal the same webhook setup any time with the `get-revenuecat-webhook-setup` MCP tool, or click **Show webhook setup** on the RevenueCat integration card in the dashboard. Both surface the existing webhook secret without rotating it, so any webhook deliveries already configured on RevenueCat's side keep working. Admin role required.

4. **Backfill existing users** with the `sync-integration` MCP tool (`provider: "revenuecat"`), or **Sync users** on the integration card in the dashboard.
...
RevenueCat fires webhooks under whatever `app_user_id` was current at purchase time, so a purchase completed before your app identified the RC SDK arrives keyed by an anonymous `$RCAnonymousID:*` ID. Owlmetry resolves these to the canonical (aliased) customer via RevenueCat's API before writing, so the subscription properties and revenue land on the real user rather than a phantom user row keyed by the anonymous ID. Anonymous customers that were never aliased to a real user are acknowledged and skipped, and transient RevenueCat API failures during resolution are answered with a retryable error so RevenueCat redelivers the webhook.
...
`$RCAnonymousID:*` IDs are excluded from both: their data belongs to the canonical (aliased) user. The daily sync job skips any such rows (reported as `skipped_anonymous` in job results), and the single-user sync rejects them with a 400.
...
2. **Create the integration** — dashboard: Integrations page → Connect Apple Search Ads, or the `add-integration` MCP tool with `provider: "apple-search-ads"`.
...
4. **Save the three IDs** — fill in the three inputs in the dashboard's "Finish setup" card and click Save IDs, or pass `client_id`, `team_id`, and `key_id` to the `update-integration` MCP tool.
5. **Pick the org** — the dashboard automatically lists accessible orgs via `GET /api/v5/acls` once the three IDs are saved; pick from the dropdown. Agents can set it directly by passing `org_id` to `update-integration` (your `orgId` is shown as "Account ID" in the ads.apple.com profile menu).

   The integration auto-enables as soon as all four IDs (`client_id`, `team_id`, `key_id`, `org_id`) are present.
6. **Backfill existing users** with the `sync-integration` MCP tool (`provider: "apple-search-ads"`) — sweeps every user with a stored `asa_campaign_id` and resolves names.
...
Pulls App Store **reviews** via Apple's App Store Connect (ASC) API. The reviews land in the `app_store_reviews` table and surface on `/dashboard/reviews`. Replaces the older iTunes RSS feed (which Apple deprecated in early 2025 and now blocks from many datacenter IPs). See [Store Reviews](/docs/concepts/reviews) for the data model.

### What it does

- Powers `app_store_connect_reviews_sync` — a system-scoped job that runs daily at 05:30 UTC, fanning out across every project with an active App Store Connect integration and calling `GET /v1/apps/{id}/customerReviews` for every Apple app that has an `apple_app_store_id`. Single-project manual triggers still work via Dashboard → Integrations → Sync and the `sync-integration` MCP tool.
...
- Dispatches an [`app.review_new`](/docs/concepts/notifications#notification-types) notification per app when at least one new review row was inserted *and* the app already had reviews on file (so the first ASC sync after setup doesn't flood inboxes). The notification body shows the count plus a snippet of the newest review.
...
1. In App Store Connect, go to **Users and Access → Integrations → App Store Connect API**, click "+" on the **Individual Keys** tab.
2. Pick the **Customer Support** role (least-privilege for read-only review access).
3. Download the `.p8` (one chance — Apple won't re-issue).
4. Copy the **Issuer ID** (top of the page) and **Key ID** (10-char alphanumeric, also embedded in the `.p8` filename).
5. Owlmetry → **Dashboard → Integrations → App Store Connect → Set up**, paste all three.
6. **Test connection** to confirm Apple accepts the key and lists your apps. **Sync reviews** to backfill.

Agents do the same through MCP: `add-integration` with `provider: "app-store-connect"` and the issuer ID, key ID, and `.p8` contents, then `sync-integration` to backfill. (Connection testing is dashboard-only.)

### Rotation

To rotate the key: re-issue a new one in App Store Connect, then call `update-integration` with the new `key_id` and `.p8` contents. Omit the `.p8` to update only the IDs (the existing `.p8` is preserved). The `.p8` is never returned by GET — Owlmetry stores it server-side and redacts it from API responses.
...
Agents manage integrations with these tools — see the [MCP integrations reference](/docs/mcp/integrations):

| Tool | Purpose |
|------|---------|
| `list-providers` | List supported providers |
| `list-integrations` | List a project's configured integrations |
| `add-integration` | Connect a provider to a project |
| `update-integration` | Update a provider's config |
| `remove-integration` | Disconnect a provider |
| `copy-integration` | Copy a configured integration to another project |
| `sync-integration` | Run a bulk sync, or sync a single user |
...
1. Use the Owlmetry MCP tools to add the RevenueCat integration to my project
   with my RC V2 secret API key (needs Customer information → Read only AND
   Project configuration → Read only at the section level, everything else No
   access).
...
4. Add Owl.setUserProperties() calls after in-app purchase events so the
   dashboard updates immediately (without waiting for the webhook) — see the
   Swift or Node SDK docs at https://owlmetry.com/docs/sdks.
```

This prompt guides the agent through all four steps: MCP integration setup, webhook configuration, backfill sync, and client-side property updates for instant feedback.
