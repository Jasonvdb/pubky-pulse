---
title: Issues
description: Automatic error detection, deduplication, and tracking with resolution workflows and notifications.
---
...
### Browser wording is canonicalized first

Chrome, Firefox, and Safari each phrase the same JavaScript fault differently, so without normalisation one broken line of code becomes three issues — one per browser, splitting the occurrence count and the affected-user count three ways.

For events with `environment: "web"` only, the message is rewritten into a browser-independent form before fingerprinting. These all become one issue:
...
The same applies to the other common shapes: "is not a function", "is not defined" / "Can't find variable:", and the cross-origin `Script error.` literal that every browser emits when a script served from another origin throws.

The property, function, or variable name is **kept** in the canonical form, so `cart.total` and `cart.items` stay separate issues. Only the wording around it collapses. Non-web events are untouched, and so are web messages that match none of the patterns — a fingerprint computed before this existed still resolves to the same issue.

### Network request errors split by endpoint

`sdk:network_request` errors (connection failures emitted by SDK network instrumentation) discriminate on `${method} ${host}${templated_path}` from the event's `_http_url` and `_http_method` custom attributes. So a connection failure to `api.stripe.com/v1/customers/<id>/subscriptions` becomes a different issue from one to your own backend, instead of all SDK-tracked network errors collapsing onto a single issue per source module.
...
Network issues get a human-readable title `Network error: METHOD host/path` (e.g. `Network error: GET api.stripe.com/v1/customers/<id>/subscriptions`).

The Web SDK's opt-in [origin URL mode](/docs/sdks/web/configuration#network-tracking-and-session-propagation)
removes paths before ingestion. Those events group at host/method granularity rather than individual
endpoint paths; invalid/non-HTTP(S) inputs omit `_http_url`. Screen templates likewise appear as the
literal configured label in dashboard filters. Neither option changes the ingest schema.

Browser exception-object suppression happens before transport and is separate from this server
fingerprinting: a factory capture followed by a global handler can emit one event, while distinct
Error objects remain eligible for the usual server grouping. See [Web error capture](/docs/sdks/web/events#capturing-exceptions).

## Session-burst aliasing

A single logical failure often produces multiple error events — a loader throws, a caller catches and logs, a `PulseOperation.fail(error:)` emits `metric:X:fail`. Each has its own `(message, source_module)`, so each hashes to a distinct fingerprint. Without aliasing, a single failure would create many separate issues.
...
- **SDK name and version**: Which Pubky Pulse SDK (e.g. `pubky-pulse-swift`) and SDK version produced the originating event. Auto-stamped by official SDKs, nullable.
- **Environment**: iOS, Android, web, backend, etc.
- **Device model and OS version**: The reporting device — hardware model and OS version on a native app, the browser (`Chrome 120`) and the OS (`macOS 10.15.7`) on web. Compare them across occurrences to tell a browser- or OS-specific fault from a universal one: all-Safari points at the browser, a spread points at your code.
...
| `resolved` | Fixed. Tagged with the app version where the fix was applied (required — see [Regression Detection](#regression-detection) below). |
...
**Web apps must set `appVersion`, and it must increase.** A browser app has no store build number to fall back on, so `Pulse.configure({ appVersion })` is the only release identifier its events carry. Leave it out and every occurrence has a null version: regression detection never fires, and the dashboard shows the issue with no version badge. Worse than leaving it out is setting it to something that does not increase — a git SHA, a build hash — because the comparator falls back to plain string ordering on non-numeric segments: whether `b12ef00` counts as newer than `a3f9c21` comes down to alphabetical order, not to which one you shipped last. Use a monotonic value: a semver string, or a numeric build counter. See [Latest Version Detection](/docs/concepts/latest-version-detection#web-apps-must-set-appversion).
...
- **Agent comments** (`author_type: "agent"`): Posted via MCP with agent key auth.
...
Two notification paths fire off issue activity, both production-only (dev issues are never surfaced to a team's inbox or email):

- **`issue.new`** — instant fan-out. The `issue_scan` job dispatches one notification per team at the end of every run summarizing all production issues created or regressed during that scan. Bypasses the per-project digest cadence so it lands in close to real time. Every eligible recipient gets a dashboard inbox row; email is off by default.
- **`issue.digest`** — per-project rollup. The `issue_notify` job assembles a periodic digest at each project's configured cadence. When produced, every eligible team member gets a dashboard inbox row; email is on by default.

Both types route through the unified [notifications dispatcher](/docs/concepts/notifications). The dashboard at `/dashboard/profile/notifications` exposes only the Email preference, which controls whether each inbox notification is also emailed. Inbox rows are not suppressed by channel preferences.

### Per-Project Alert Frequency

The `issue_alert_frequency` setting controls **only the digest cadence** — `issue.new` fires every scan regardless.
...
Configure the frequency via the project settings in the dashboard or the API (`PATCH /v1/projects/:id` with `issue_alert_frequency`). Setting it to `none` prevents digest production, so no digest inbox rows or emails are created. Instant `issue.new` notifications still fire and always create inbox rows; each user's Email preference only controls whether those notifications are also emailed.
...
Both permissions are included in the default agent key permissions. Permissions are necessary but not sufficient: status changes and merges also require ownership of the project, while commenting deliberately does not — see [Authentication & Access Control](/docs/concepts/authentication#comments).
