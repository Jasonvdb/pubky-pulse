...
Both types route through the unified [notifications dispatcher](/docs/concepts/notifications), so each recipient's per-channel toggles at `/dashboard/profile/notifications` decide whether a given delivery becomes an inbox row, an email, a mobile push, or some combination.
