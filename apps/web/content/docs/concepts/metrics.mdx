...
```text title="Add structured metrics with your coding agent"
Add Pubky Pulse structured metrics to this project. Connect the Pubky Pulse MCP
...
- For each one, create a metric definition on the server with the
  create-metric MCP tool, then use the SDK's lifecycle API for
  timed operations.
...
The SDKs generate and manage tracking IDs automatically via the `PulseOperation` object returned by `startOperation()`.
...
Definitions are managed through the [dashboard](/docs/concepts/projects-and-apps), [MCP](/docs/mcp/metrics), or the [API](/docs/api-reference/metrics).
...
- `device_model` -- by device hardware, or by browser for web events (`Chrome 120`)
- `os_version` -- by OS version, or by OS name and version for web events (`macOS 10.15.7`)
- `environment` -- by runtime platform
- `time:hour`, `time:day`, `time:week` -- by time bucket

Metric events copy `device_model` and `os_version` from the event that produced them, so grouping a browser app's metrics by `device_model` answers "is checkout slow only in Safari?" without any extra instrumentation. Group by `environment` first when a project mixes web with native apps -- otherwise a browser and a phone land in the same table with no way to tell them apart.

### Filtering

Queries accept optional filters: `since`, `until`, `app_id`, `app_version`, `device_model`, `os_version`, `user_id`, `environment`, and `data_mode`. `device_model` and `os_version` are exact matches against the stored value, so filter a browser app on the browser string as it is recorded (`Chrome 120`, not `Chrome`).
