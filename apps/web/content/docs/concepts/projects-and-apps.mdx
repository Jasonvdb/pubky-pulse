...
- [Issue alerting](/docs/concepts/issues) -- per-project `issue_alert_frequency` (`none`, `hourly`, `6_hourly`, `daily` (default), `weekly`) controls the digest cadence only; instant `issue.new` push fan-out fires every scan regardless
