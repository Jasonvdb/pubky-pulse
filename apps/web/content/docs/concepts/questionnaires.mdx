...
Questionnaires are short, structured surveys you build once on the server and present in-app via the Swift and Android SDKs. Where [feedback](./feedback) is a single free-text message, a questionnaire walks the user through a sequence of typed questions — rating scales, NPS, multiple choice, free text — and rolls each answer up in dashboard analytics.

## How It Works

1. Create a questionnaire via the dashboard or the `create-questionnaire` MCP tool. Pick an immutable slug like `post-onboarding`. The schema is JSON: up to 30 questions, mix of `text`, `single_choice`, `multi_choice`, `rating` (1–5), `nps` (0–10).
2. In your app, add the SDK's questionnaire gate referencing the slug — in SwiftUI, e.g. `.pulseQuestionnaire(slug: "post-onboarding", trigger: .afterLaunches(3))`.
...
The `.pulseQuestionnaire(trigger:)` modifier accepts a composable trigger. Conditions are ANDed — all must be true for the sheet to present.

```swift
// Show on third launch
.pulseQuestionnaire(slug: "after-3-launches", trigger: .afterLaunches(3))
...
Each response gets a comment thread mirroring [feedback comments](./feedback#comments) — any team member can comment, only the author may edit, and a comment is deleted by its author or, for moderation, by a human project owner. An agent key may delete only the comments that exact key wrote. See [Authentication & Access Control](/docs/concepts/authentication#comments).
...
- Deleting a questionnaire or a questionnaire response is **human-only**. Agent keys get 403 — by design, to prevent an agent from removing live SDK integrations or real user submissions

Permissions are necessary but not sufficient: creating, updating and deleting questionnaires, and changing or deleting responses, also require ownership of the project, while commenting deliberately does not — see [Authentication & Access Control](/docs/concepts/authentication#project-ownership).
...
- [Android SDK questionnaires](/docs/sdks/android/questionnaires) — Compose gate + manual presentation
