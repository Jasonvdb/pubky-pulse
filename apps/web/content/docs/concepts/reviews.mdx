...
| `app_store_connect_reviews_sync` | daily 05:30 UTC (system) | App Store Connect API | Fans out across every project with an active App Store Connect integration and pulls individual reviews via `GET /v1/apps/{id}/customerReviews` for every Apple app in those projects that has an `apple_app_store_id`. |

The reviews job runs daily and can also be triggered manually for a single project — manual runs hit the same handler, scoped to one project. Manual triggers from Dashboard → Integrations → Sync reviews, or via the `sync-integration` MCP tool. Each project still requires its own active App Store Connect integration to be included in the daily fan-out.

Aggregate ratings keep flowing automatically once an Apple app has a bundle ID — no integration setup needed. ASC is only required for individual review ingestion. You can also manually trigger a ratings sync (e.g. right after creating a new app) via Dashboard → project → Sync ratings or the `sync-app-ratings` MCP tool.

### Notifications on new ratings & reviews

Both jobs fan out to every team member via the [notification dispatcher](/docs/concepts/notifications):
...
Per-channel toggles (in-app, email, mobile push) live under each user's [notification preferences](/docs/concepts/notifications#per-user-preferences). `app.rating_changed` defaults to in-app + mobile push (email off); `app.review_new` defaults to all three channels on.
...
- **MCP**: `list-app-ratings`, `list-ratings-by-country`, and `sync-app-ratings` (admin-only manual trigger).
...
You can reply to an Apple review directly from the dashboard, MCP, or the iOS companion app — Owlmetry calls App Store Connect's `customerReviewResponses` endpoint, persists the resulting reply ID + state, and shows it on every surface so you can see at a glance which reviews you've already responded to.
...
**Deleting**: removes the reply from the public App Store listing. **Irrecoverable** — the only way back is to post a new reply. The dashboard requires you to type `delete` to confirm, the iOS app uses a destructive alert, and the MCP `delete-review-response` tool description leads with a destructive-action warning.
...
- **Daily sync hasn't run yet for a brand-new app** — trigger `sync-app-ratings` to populate immediately.
