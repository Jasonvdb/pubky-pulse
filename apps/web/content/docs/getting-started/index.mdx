...
Owlmetry is designed to be set up by your coding agent. The [MCP endpoint](/docs/mcp/setup) is the agent interface: configure a URL once in your editor and your agent gets 62 tools instantly, with no installation and no updates to keep on top of.
...
```
Set up Owlmetry in this project. Use the Owlmetry MCP tools to create a
project and an app, then instrument the code with the SDK for this
platform.
```

Supported clients read the `owlmetry://guide` resource automatically on connection, so your agent already knows the concepts, resource hierarchy, and conventions — it takes it from there: `create-project`, `create-app`, then installing the SDK and wiring up event tracking. The SDK integration guides live at [/docs/sdks](/docs/sdks).

**Tip:** Add this line to your project's `CLAUDE.md`, `AGENTS.md`, or whatever default instructions file your coding agent uses:

```markdown
### Owlmetry
Use the Owlmetry MCP tools for any analytics work — read the `owlmetry://guide`
resource first, and see https://owlmetry.com/docs/sdks for SDK integration.
```

This ensures every future session knows Owlmetry is integrated and picks up the right context automatically — including the full tool surface, funnel design rules, and lifecycle metric patterns.
...
| [Android SDK](/docs/sdks/android) | Android (Kotlin / Compose) | Gradle (`com.owlmetry:owlmetry-android`) |
