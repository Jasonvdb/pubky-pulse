...
Listing and downloading attachments spans every project in the configured team. **Deleting one is human-only**: `delete-attachment` returns `403` for every agent key, even one whose creator owns the project. See [Setup](/docs/mcp/setup#access-model-read-wide-write-narrow).
...
⚠️ **Human-only — agent calls receive `403`.** Attachment deletion is one of the destructive actions reserved for people; ask a project owner to do it from the dashboard or the REST API.

Soft-deletes an event attachment, which is hard-deleted from disk 7 days later by the `attachment_cleanup` job. Used once a bug is confirmed resolved, to free the project's storage quota.
