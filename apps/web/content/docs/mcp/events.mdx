...
import Concept from '../../_snippets/events.mdx';
...
| `screen_name` | string | No | Filter by screen name. Exact or path-prefix: `/checkout` also returns `/checkout/payment`, but not `/checkout-abandoned` |
| `device_model` | string | No | Filter by device model (`iPhone15,2`), or by browser on web (`Chrome 120`) |
| `os_version` | string | No | Filter by OS version (`18.0`), or by OS name and version on web (`macOS 10.15.7`) |
...
`device_model`, `os_version`, and `screen_name` hold different things per platform — the browser, the OS name and version, and the screen name (URL path by default) on `web`; the hardware model, the OS version, and the screen name on native. Both device filters are exact matches, so filter on the value as recorded (`Chrome 120`, not `Chrome`). See [Field semantics by platform](/docs/concepts/events#field-semantics-by-platform).
