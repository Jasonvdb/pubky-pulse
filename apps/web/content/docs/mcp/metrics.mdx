...
import Concept from '../../_snippets/metrics.mdx';

<Concept />

## list-metrics

List metric definitions. Pass `project_id` for a single project, or `team_id` to list every metric across every accessible project in the team (mutually exclusive). Each row carries `project_id` so you can tell which project owns it.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `project_id` | UUID | One of | The project ID — single-project view |
| `team_id` | UUID | One of | The team ID — lists across all accessible projects (mutually exclusive with `project_id`) |
...
Create a metric definition. Definitions are optional — metric events are ingested and queryable by slug either way — but defining a metric gives you a human-readable name and documentation in the dashboard. Requires `metrics:write` permission **and** project ownership.
...
Update a metric definition. Requires `metrics:write` permission **and** project ownership.
...
Soft-delete a metric definition. Existing metric events are not removed. Requires `metrics:write` permission **and** project ownership. Deleting a metric definition is one of the destructive operations agent keys *are* allowed to perform.
...
| `device_model` | string | No | Filter by device model, or by browser on web (`Chrome 120`) |
| `os_version` | string | No | Filter by OS version, or by OS name and version on web (`macOS 10.15.7`) |
...
| `device_model` | Group by device model — by browser for web events |
| `os_version` | Group by OS version — by OS name and version for web events |
