...
Reading questionnaires and responses spans every project in the configured team. Creating, editing and triaging them requires the human who created this key to currently own the project, re-read on every request; a refusal is `403 Requires project ownership`. Commenting on a response is the exception and needs only `questionnaires:write`. See [Setup](/docs/mcp/setup#access-model-read-wide-write-narrow).

Deleting a questionnaire **or a response** is human-only: agent calls receive `403` even when the creator owns the project.
...
| `delete-questionnaire` | ⚠️ Human-only — agent calls receive 403 |
...
2. **Wait for responses** — your app's questionnaire gate (`.pulseQuestionnaire(slug:trigger:…)` in SwiftUI, `PulseQuestionnaireGate` in Compose) fetches and presents the survey on the configured trigger, saving a draft after every Next tap.
