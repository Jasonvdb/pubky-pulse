...
2. **Package ID** — automatically read from `context.packageName` (e.g. `com.example.app`) and sent. It must match the client key's registered Android app; developers do not configure it manually.
...
| `PulseConfigurationError.MissingBundleId` | The package name could not be determined (rare — empty `context.packageName`). |
