...
**Automatic network-request instrumentation** is not yet provided by the Android SDK — `networkTrackingEnabled` is reserved and currently a no-op. Track HTTP failures explicitly with `Owl.error` in the meantime.
