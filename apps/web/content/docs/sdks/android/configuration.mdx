---
title: Configuration
description: Configure the Android SDK with Pulse.configure() — context, endpoint, API key, and options.
---

import EndpointDefault from '../../../_snippets/sdk-endpoint.mdx';

Call `Pulse.configure()` once, as early as possible — in your `Application` subclass's `onCreate()`. This initializes the SDK, starts a new session, and begins capturing events.

## Basic Setup

Create (or extend) an `Application` subclass and call `configure()` in `onCreate()`:

```kotlin
import android.app.Application
import org.pubky.pulse.android.Pulse
import org.pubky.pulse.android.PulseConfigurationError

class MyApp : Application() {
    override fun onCreate() {
        super.onCreate()
        try {
            Pulse.configure(
                context = this,
                apiKey = "pulse_client_...",
            )
        } catch (e: PulseConfigurationError) {
            android.util.Log.e("PubkyPulse", "configuration failed", e)
        }
        Pulse.info("app_launched")
    }
}
```

Register the `Application` subclass in your manifest:

```xml
<application
    android:name=".MyApp"
    ... >
```

> **Do not defer configuration** to a later point (the first `Activity`, after async setup, or after user consent). The SDK measures app launch time (`_launch_ms`) from process start to the `configure()` call, so placing it early gives an accurate cold-start metric — and ensures no events are dropped before configuration.

<EndpointDefault />

## Parameters

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `context` | `Context` | required | An Android `Context`. `configure()` uses `applicationContext` internally, so passing `this` from `Application` is correct. |
| `endpoint` | `String` | `"https://ingest.pubkypulse.com"` | Your Pubky Pulse ingest server URL. Omit it to use Pubky's hosted instance; self-hosters must set their own. |
| `apiKey` | `String` | required | Client API key. Must start with `pulse_client_`. |
| `flushOnBackground` | `Boolean` | `true` | Automatically flush buffered events when the app enters the background. |
| `compressionEnabled` | `Boolean` | `true` | Gzip-compress request bodies before sending. |
| `networkTrackingEnabled` | `Boolean` | `true` | Reserved for automatic HTTP request tracking (not yet implemented on Android). |
| `consoleLogging` | `Boolean` | `true` | Echo events to Logcat. Set to `false` to suppress. |

## Full Example with All Options

```kotlin
Pulse.configure(
    context = this,
    endpoint = "https://ingest.yourdomain.com",  // required when you run your own instance
    apiKey = "pulse_client_...",
    flushOnBackground = true,
    compressionEnabled = true,
    networkTrackingEnabled = true,
    consoleLogging = true,
)
```

## What Happens on Configure

1. **Session ID** — a fresh UUID is generated to group all events from this launch. Readable at runtime via `Pulse.sessionId`.
2. **Package ID** — automatically read from `context.packageName` (e.g. `com.example.app`) and sent when available. With the compatible server, it must match the client key's registered Android app; developers do not configure it manually.
3. **Debug mode** — `is_dev` is set to `true` when the host app is debuggable (the `FLAG_DEBUGGABLE` manifest flag — true for debug builds, false for release).
4. **Launch time** — the SDK measures the time from process start (`Process.getStartUptimeMillis()`) to the `configure()` call and includes it as `_launch_ms` on the `session_started` event. Place `configure()` as early as possible for an accurate cold-start measurement.
5. **Session event** — a `sdk:session_started` event is emitted immediately with the `_launch_ms` attribute.
6. **Lifecycle observer** — if `flushOnBackground` is `true`, the SDK registers a process-`LifecycleObserver` to flush on background.
7. **Network monitor** — starts tracking connection type (wifi, cellular, offline).
8. **Startup identity claim** — if a real user ID was saved on a previous launch and differs from the anonymous ID, the SDK fires an idempotent claim so anonymous events sent before the user was known attribute to the real user.

## Configuration Errors

`Pulse.configure()` throws `PulseConfigurationError` — a sealed exception hierarchy with three cases:

| Error | When it occurs |
|-------|---------------|
| `PulseConfigurationError.InvalidEndpoint` | The endpoint string is not a valid absolute http(s) URL with a host. |
| `PulseConfigurationError.InvalidApiKey` | The API key does not start with `pulse_client_`. |
| `PulseConfigurationError.MissingBundleId` | Older releases throw when the package name is unavailable. The forthcoming best-effort release retains this class for compatibility but omits unavailable metadata instead. |

Omitting unavailable identifiers requires the compatible server to be deployed first; see the [app association rules](/docs/mcp/apps).

Wrap the call in `try`/`catch` as shown above.

## Not yet on Android

**Automatic network-request instrumentation** is not yet provided by the Android SDK — `networkTrackingEnabled` is reserved and currently a no-op. Track HTTP failures explicitly with `Pulse.error` in the meantime.

## Calling Configure More Than Once

Calling `configure()` again is safe. The SDK shuts down the previous transport, stops the old lifecycle observer, and starts fresh with a new session ID — emitting a new `sdk:session_started` event.

## Shutdown

In most cases the SDK handles cleanup automatically via background-flush on app background. If you need to guarantee delivery at a specific point (e.g. before a deliberate teardown in a test harness), call `shutdown()`:

```kotlin
Pulse.shutdown()  // suspend — flushes buffered events and stops the background-flush observer
```

`shutdown()` is a suspend function; call it from a coroutine. The SDK can be `configure()`d again afterward.

## Next Steps

- [Events](/docs/sdks/android/events) — start logging events
- [Identity](/docs/sdks/android/identity) — connect events to real users
- [Screen Tracking](/docs/sdks/android/screen-tracking) — add automatic screen view tracking
