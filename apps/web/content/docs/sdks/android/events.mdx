...
Events are the core unit of data in Pubky Pulse. The Android SDK provides four log levels, each designed for a different class of information. For a broader overview of the event model, see [Events](/docs/concepts/events).
...
```kotlin
Pulse.debug("Cache hit for user preferences", screenName = "Home", attributes = mapOf("key" to "user_prefs"))
```
...
```kotlin
Pulse.warn("Slow network response", attributes = mapOf("latency_ms" to "1200", "endpoint" to "/api/feed"))
```
...
```kotlin
Pulse.error("Failed to load profile image", screenName = "Profile", attributes = mapOf("reason" to "timeout"))
```
...
try {
    ...
    Pulse.error(e, message = "while uploading photos", screenName = "PhotoConverterScreen")
}
```
...
| `attachments` | Optional file attachments (logs, screenshots, etc.). Most useful on `Pulse.error()`. See [File Attachments](#file-attachments). |
...
Pulse.info("Draft created", attributes = mapOf(
    "context" to "createDraft",
    "contractId" to contractId,
))
```

If `contractId` is `null`, only `context` reaches the dashboard. The same applies to every `Pulse` / `PulseOperation` method that takes `attributes`.
...
`PulseAttachment` has two factory entry points:
...
PulseAttachment.file(inputFile, name = "input.heic", contentType = "image/heic")

// From in-memory bytes (name is required)
PulseAttachment.bytes(debugJson, name = "debug.json", contentType = "application/json")
```
...
try {
    ...
    Pulse.error(
        ...
        attachments = listOf(PulseAttachment.file(inputFile)),
    )
}
```
...
| Event | Level | When |
|-------|-------|------|
| `sdk:session_started` | info | On `Pulse.configure()`. Includes `_launch_ms` (process start → configure). |
| `sdk:app_foregrounded` | info | When the app enters the foreground. |
| `sdk:app_backgrounded` | info | When the app enters the background. |
| `sdk:screen_appeared` | debug | When a composable with `Modifier.pulseScreen()` enters the composition. See [Screen Tracking](/docs/sdks/android/screen-tracking). |
...
Every event the Android SDK emits is stamped with a session ID generated on `Pulse.configure()` and accessible via `Pulse.sessionId`. When the app calls your backend, forward this value as a request header so events logged by the backend can be tagged with the same session ID:

```kotlin
val sessionId = Pulse.sessionId
if (sessionId != null) {
    requestBuilder.addHeader("X-Pulse-Session-Id", sessionId)
}
```
