---
title: Installation
description: Add the Pubky Pulse Android SDK to your project via Gradle and Maven Central.
---

The Pubky Pulse Android SDK is published to Maven Central as two artifacts. The core module has a single runtime dependency (`kotlinx-coroutines`); add the optional Compose module only if you want the drop-in UI.

| Artifact | When to add |
|----------|-------------|
| `org.pubky.pulse:pulse-android` | Always — the core SDK. |
| `org.pubky.pulse:pulse-android-compose` | Only if you use `PulseFeedbackView`, `PulseQuestionnaireGate` / `PulseQuestionnaireView`, or the `Modifier.pulseScreen()` modifier. |

## Version catalog (recommended)

If your project uses a version catalog (`gradle/libs.versions.toml`), add the version and library entries there:

```toml
[versions]
pulse = "0.1.0"

[libraries]
pulse-android = { module = "org.pubky.pulse:pulse-android", version.ref = "pulse" }
pulse-android-compose = { module = "org.pubky.pulse:pulse-android-compose", version.ref = "pulse" }
```

Then in your app module's `build.gradle.kts`:

```kotlin
dependencies {
    implementation(libs.pulse.android)
    // Optional — only for the drop-in Compose UI and the pulseScreen() modifier:
    implementation(libs.pulse.android.compose)
}
```

## Direct coordinates

If the project declares dependencies directly (no catalog), add to the app module's `build.gradle.kts`:

```kotlin
dependencies {
    implementation("org.pubky.pulse:pulse-android:0.1.0")
    // Optional Compose UI:
    implementation("org.pubky.pulse:pulse-android-compose:0.1.0")
}
```

The Groovy `build.gradle` equivalent:

```groovy
dependencies {
    implementation 'org.pubky.pulse:pulse-android:0.1.0'
    implementation 'org.pubky.pulse:pulse-android-compose:0.1.0'
}
```

> For the latest version, see [releases](https://github.com/Jasonvdb/pubky-pulse-android/releases/latest). Pin a version for reproducible builds.

Make sure Maven Central is a configured repository — it almost always already is, in `settings.gradle.kts` under `dependencyResolutionManagement { repositories { mavenCentral() } }`.

## Permissions

The core module merges two install-time, no-prompt permissions into your app's manifest: `INTERNET` and `ACCESS_NETWORK_STATE`. No runtime permission request is needed.

## Verify the Installation

After editing the Gradle files, sync and build:

```bash
./gradlew :app:assembleDebug --quiet
# or, to just resolve dependencies without a full build:
./gradlew :app:dependencies --configuration debugRuntimeClasspath | grep pulse-android
```

Adjust `:app` to the actual application module name if it differs. Unresolved-import warnings in the editor before the first Gradle sync are expected.

## Requirements

| Requirement | Minimum |
|-------------|---------|
| Android | 7.0 (API 24) |
| Kotlin | 2.0+ |
| AGP | 8.7+ |
| JDK | 17+ |

## Next Steps

Once the dependency resolves and your project builds, proceed to [Configuration](/docs/sdks/android/configuration) to initialize the SDK.
