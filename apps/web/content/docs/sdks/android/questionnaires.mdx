...
description: Render in-app surveys with PulseQuestionnaireGate, PulseQuestionnaireView, and Pulse.saveQuestionnaireResponse.
---
...
The Compose UI (`PulseQuestionnaireGate` / `PulseQuestionnaireView`) lives in the `pulse-android-compose` artifact; the programmatic APIs (`Pulse.fetchQuestionnaire`, `Pulse.saveQuestionnaireResponse`, `Pulse.dismissQuestionnaires`) live in the core module.

## Prerequisite

Create a questionnaire with a slug first — via the [dashboard](/dashboard/questionnaires) or [MCP](/docs/mcp/questionnaires) (`create-questionnaire`). The SDK only reads and saves — it doesn't define questionnaires. The slug is immutable after creation.
...
- **Don't ask again** → confirmation dialog, then writes the global-dismiss flag (`Pulse.dismissQuestionnaires()`); never asked again
...
Compose has no modifier-driven presentation, so the gate is a **wrapper composable** (`PulseQuestionnaireGate`) — the analog of Swift's `.pulseQuestionnaire(...)` modifier. Render your screen as its `content` and the gate overlays the questionnaire once the trigger fires and the server returns an eligible spec.
...
| `strings` | `.DEFAULT` | Override consent + flow copy via `PulseQuestionnaireStrings.DEFAULT.with(...)`. The spec's `description` wins over `strings.consentBody` when present. |
| `onSubmitted` | `null` | Fires once on the call that flips the response to submitted. Receives `PulseQuestionnaireReceipt`. |
...
```kotlin
import org.pubky.pulse.android.PulseQuestionnaireTrigger
import org.pubky.pulse.android.PulseQuestionnaireCondition
...
- `PulseQuestionnaireCondition.Launches(atLeast)` — total `Pulse.configure(...)` calls since install (one bump per process)
- `PulseQuestionnaireCondition.Foregrounds(atLeast)` — total foreground transitions since install
- `PulseQuestionnaireCondition.DaysSinceFirstLaunch(atLeast)`
- `PulseQuestionnaireCondition.HoursSinceFirstLaunch(atLeast)`

Shortcuts on `PulseQuestionnaireTrigger`:
...
- `.manual` — never auto-trigger; present `PulseQuestionnaireView` directly
...
`Pulse` exposes the persistent counters that drive the trigger evaluator — useful for debugging or building your own gates:

```kotlin
val launches = Pulse.launchCount        // total Pulse.configure(...) calls
val foregrounds = Pulse.foregroundCount // total foreground transitions
val installedAt = Pulse.firstLaunchAt   // Long? — epoch millis of the first configure()
```

## Manual presentation

For ad-hoc triggers ("show after the user finishes the import wizard"), call `Pulse.fetchQuestionnaire` and present `PulseQuestionnaireView` yourself. `fetchQuestionnaire` returns a result whose `.questionnaire` is null when the user is ineligible (`.ineligibleReason` carries `alreadyResponded`, `globallyDismissed`, or `inactive`) and whose `.inProgress` carries any unsubmitted draft to resume.
...
var inProgress by remember { mutableStateOf<PulseQuestionnaireDraft?>(null) }
var show by remember { mutableStateOf(false) }
val scope = rememberCoroutineScope()

Button(onClick = {
    scope.launch {
        val result = runCatching { Pulse.fetchQuestionnaire("post-import") }.getOrNull()
...
`PulseQuestionnaireView`'s public `showsConsent` defaults to `false` — the consumer is already deciding to show it, so the step flow opens directly. Pass `showsConsent = true` if you want the consent prompt in front.

## Programmatic dismissal

To opt the user out of all future questionnaires from code (e.g. a settings toggle), call `Pulse.dismissQuestionnaires()`:

```kotlin
scope.launch {
    runCatching { Pulse.dismissQuestionnaires() }
}
```
...
`PulseQuestionnaireView` and `PulseQuestionnaireGate` both accept a `PulseQuestionnaireStrings` parameter. Override individual strings with `PulseQuestionnaireStrings.DEFAULT.with(...)`:

```kotlin
PulseQuestionnaireGate(
    slug = "post-onboarding",
    trigger = PulseQuestionnaireTrigger.afterLaunch,
    strings = PulseQuestionnaireStrings.DEFAULT.with(
        submitButton = "Send",
...
If you're rendering your own survey UI instead of `PulseQuestionnaireView`, call `Pulse.saveQuestionnaireResponse(slug, answers, isComplete)` to persist progress. Pass `isComplete = false` after each answer to save a draft (idempotent — the server upserts by `(project, slug, user_id)`, so the SDK doesn't track a response id across calls); pass `isComplete = true` on the final submit to flip `submitted_at` and fire the team notification:

```kotlin
import org.pubky.pulse.android.PulseQuestionnaireAnswerValue

// Save a draft after the user answers Q2
Pulse.saveQuestionnaireResponse(
    slug = "post-import",
    answers = mapOf(
        "q1" to PulseQuestionnaireAnswerValue.RatingValue(5),
        "q2" to PulseQuestionnaireAnswerValue.TextValue("Nice!"),
    ),
    isComplete = false,
)
...
Answer value types (all sealed subclasses of `PulseQuestionnaireAnswerValue`):
...
Every save sends the full accumulated answer set; the server merges, so going Back and re-answering an earlier question just overwrites that key. `PulseQuestionnaireReceipt.wasSubmitted` is true exactly once per response — on the call that flipped `submitted_at` — making it safe to gate success-state UI on without separate bookkeeping.

## Errors

`Pulse.fetchQuestionnaire` and `Pulse.saveQuestionnaireResponse` throw `PulseQuestionnaireError`:

- `PulseQuestionnaireError.NotConfigured` — `Pulse.configure(...)` hasn't been called yet
- `PulseQuestionnaireError.SlugNotFound` — the slug doesn't exist in the project (developer error — check spelling)
- `PulseQuestionnaireError.ServerError` / `PulseQuestionnaireError.TransportFailure` — network / 5xx
