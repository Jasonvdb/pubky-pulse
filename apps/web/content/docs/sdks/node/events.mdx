...
```typescript
Pulse.info("Order placed", { order_id: "abc-123" });
Pulse.debug("Cache miss for user lookup", { cache_key: "user:42" });
Pulse.warn("Retry attempt on payment gateway", { attempt: "2" });
Pulse.error("Failed to send notification email", { email: "user@example.com" });
```

## Reporting an `Error` Value

`Pulse.error` is overloaded — pass a string message (logger-style) or pass an `Error` value (exception-style). When given an Error, the SDK extracts the type name, full stack, `Error.cause` chain, `AggregateError` children, and Node `code/syscall/path` fields into reserved `_error_*` attributes. The server's issue tracker uses `_error_type` as a fingerprint discriminator so different error classes with the same wording stay on separate issues.

```typescript
try {
  await processOrder(req.body);
} catch (err) {
  Pulse.error(err);                                 // auto-derives message
  Pulse.error(err, "while processing order");       // caller-supplied context
  Pulse.error(err, "while processing order", {      // + extra attributes
...
The third `options` argument of every log method accepts an `attachments` array. Each entry is a file from disk (`path`) or in-memory bytes (`buffer`) that will be uploaded alongside the event and viewable from the dashboard or MCP:

```typescript
try {
  await PdfParser.parse(inputPath);
} catch (err) {
  Pulse.error("pdf parse failed", { error: String(err) }, {
    attachments: [
...
Uploads run on a serial queue and are awaited by `Pulse.flush()` and `Pulse.shutdown()`. Use attachments sparingly -- each project has a default 5 GB storage quota with a 250 MB per-user bucket. See [Attachments](/docs/concepts/attachments) for details on quotas and viewing attached files.

## User-Scoped Events

Use `Pulse.withUser()` to create a scoped instance that tags every event with a user ID:

```typescript
const pulse = Pulse.withUser("user_42");
pulse.info("Profile updated");
pulse.warn("Rate limit approaching", { requests: "95" });
```
...
app.use((req, res, next) => {
  ...
  req.pulse = userId ? Pulse.withUser(userId) : Pulse;
  next();
});

app.post("/api/orders", (req, res) => {
  req.pulse.info("Creating order", { items: String(req.body.items.length) });
  // ...
  req.pulse.info("Order created", { order_id: order.id });
  res.json(order);
});
```
...
1. Calling `Pulse.info()` (or any log method) creates an event and adds it to the buffer
...
For serverless functions where the process may terminate before the timer fires, use [`Pulse.wrapHandler()`](/docs/sdks/node/serverless) to flush after each invocation.
