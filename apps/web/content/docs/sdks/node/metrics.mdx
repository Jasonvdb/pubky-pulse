...
```typescript
const op = Pulse.startOperation("order-processing", { order_id: "abc-123" });
...
`Pulse.startOperation(metric, attrs?)` returns a `PulseOperation` object and emits a `start` phase event immediately.
...
Call exactly one of these methods on the returned `PulseOperation`:
...
Use `Pulse.recordMetric()` for measurements that do not have a start/end lifecycle:

```typescript
Pulse.recordMetric("daily-active-users", { count: String(dauCount) });
Pulse.recordMetric("queue-depth", { size: String(queue.length) });
```
...
Pulse.startOperation("API Request Time");
```

Define your metric slugs in the dashboard or with the `create-metric` MCP tool before sending events to get the most out of metric aggregation queries. See [Structured Metrics](/docs/concepts/metrics) for details on how the server processes metric events.
...
app.post("/api/orders", async (req, res) => {
  ...
  const op = pulse.startOperation("create-order", {
