...
2. **Bundle ID** -- automatically detected and sent. It must match the client key's registered Apple app; developers do not configure it manually. Watch apps prefer their companion app's identifier when present, otherwise their own.
...
| `missingBundleId` | The bundle identifier is nil or empty. This can happen in command-line tools or certain test environments. |
