...
description: What ships in the SDK, what you need to declare in your app's App Privacy settings, and how Owlmetry handles tracking, IDFA, and required-reason APIs.
---
...
|---|---|---|
| `NSPrivacyAccessedAPICategoryUserDefaults` | `CA92.1` | Persist the real user identifier (set via `Owl.setUser`) in your app's own UserDefaults suite |
...
After the one-time Nutrition Label update on your next submission, every subsequent submission is unchanged. The manifest ships with the SDK and updates automatically when you upgrade Owlmetry. You only need to revisit your app's App Privacy settings if you start using a new Owlmetry feature that introduces a category you have not yet declared (for example, enabling `OwlFeedbackView` contact fields for the first time).
