...
description: Render in-app surveys with PulseQuestionnaireView and the .pulseQuestionnaire view modifier.
---
...
Create a questionnaire with a slug first — via the [dashboard](/dashboard/questionnaires) or [MCP](/docs/mcp/questionnaires) (`create-questionnaire`). The SDK only reads and saves — it doesn't define questionnaires.
...
`Pulse.fetchQuestionnaire(slug:)` returns a `PulseQuestionnaireFetchResult` that bundles the spec with an optional `inProgress: PulseQuestionnaireDraft` — populated when the current user already has a draft response on the server (`submitted_at` still null). The auto-trigger modifier and the manual `PulseQuestionnaireView` both accept the draft and resume the flow at the first unanswered question with the prior answers pre-filled; the consent detent is skipped on resume because the user already opted in earlier. Abandoned drafts are soft-deleted after 90 days of inactivity by the `questionnaire_draft_cleanup` background job.
...
The flag is set at authoring time on the server (dashboard or MCP); the SDK simply honours whatever the schema says.

## Auto-trigger view modifier

The easiest path is to attach `.pulseQuestionnaire(slug:…)` to a long-lived view (often the root of your app or a tab's content):
...
- `.launches(atLeast: Int)` — total `Pulse.configure(...)` calls since install (one bump per process)
...
- `.manual` — never auto-trigger; consumer presents `PulseQuestionnaireView` directly

OR-logic isn't built in. If you need it, use the `isEligible` closure or attach two `.pulseQuestionnaire(...)` modifiers with different slugs.
...
```swift
.pulseQuestionnaire(slug: "...", trigger: .afterLaunch, tint: .orange)
```
...
`onCancel` fires when the user taps Cancel mid-flow or "Maybe later" on the consent prompt — answers entered up to that point persist as a draft on the server, but no `onSubmitted` fires. `onDismissed` fires when the user taps "Don't ask again" → confirm, after the server-side global-dismiss flag is written. `onSubmitted` fires only on the final Submit; the `PulseQuestionnaireReceipt` carries `wasSubmitted: Bool` which is `true` for that one call (it flipped server-side `submitted_at` from null to non-null) and `false` for the per-question draft saves leading up to it.

## Manual presentation

For ad-hoc triggers ("show this survey after the user finishes the import wizard"), call `Pulse.fetchQuestionnaire` and present `PulseQuestionnaireView` yourself. `fetchQuestionnaire` returns a `PulseQuestionnaireFetchResult` whose `.questionnaire` is nil when the user is ineligible (`.ineligibleReason` carries `alreadyResponded`, `globallyDismissed`, or `inactive`) and whose `.inProgress` carries any unsubmitted draft to resume. The public init defaults `showsConsent: false` — the consumer is already deciding to show it, so the step flow opens directly. Pass `showsConsent: true` if you want the small-detent prompt in front:

```swift
@State private var presentation: (PulseQuestionnaire, PulseQuestionnaireDraft?)?
...
Toggle("Show in-app surveys", isOn: $surveysEnabled)
    .onChange(of: surveysEnabled) { _, newValue in
        if !newValue {
            Task { try? await Pulse.dismissQuestionnaires() }
        }
    }
```
...
`PulseQuestionnaireView` and the gate modifier both accept a `PulseQuestionnaireStrings` parameter. Every user-facing string is a `LocalizedStringResource` resolving against the SDK's bundled `Localizable.xcstrings` catalog. Override individual strings with `.default.with(...)`:
...
```swift
let launches = Pulse.launchCount       // total Pulse.configure(...) calls
let foregrounds = Pulse.foregroundCount // total foreground transitions
let installedAt = Pulse.firstLaunchAt   // Date? — first Pulse.configure(...) timestamp
```
...
Every save sends the full accumulated answer set; the server merges, so going Back and re-answering an earlier question just overwrites that key in the saved row. `PulseQuestionnaireReceipt.wasSubmitted` is true exactly once per response — on the call that flipped `submitted_at` — making it safe to gate success-state UI on without separate bookkeeping.

## Errors

`Pulse.fetchQuestionnaire` and `Pulse.saveQuestionnaireResponse` throw `PulseQuestionnaireError`:

- `.notConfigured` — `Pulse.configure(...)` hasn't been called yet
...
`fetchQuestionnaire` doesn't throw for ineligible users — it returns a `PulseQuestionnaireFetchResult` whose `.questionnaire` is nil and whose `.ineligibleReason` (`alreadyResponded` / `globallyDismissed` / `inactive`) explains why.
