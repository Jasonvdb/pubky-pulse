---
title: Requirements
description: Server requirements for self-hosting Pubky Pulse — OS, Node.js, PostgreSQL, and hardware.
---

Server requirements for self-hosting Pubky Pulse.
...
If you plan to use [event attachments](/docs/concepts/attachments) (SDK file uploads like logs or screenshots), budget additional disk for `PULSE_ATTACHMENTS_PATH` (default `/opt/pubky-pulse-attachments`). Attachment bytes live on disk, not in Postgres, and are governed by per-project and per-user quotas — default 5 GB per project ceiling and 250 MB per user bucket.
...
| pnpm | 10 | Installed globally via `npm install -g pnpm` |
| pm2 | Latest | Installed globally via `npm install -g pm2`. Process manager for the `pulse-api` and `pulse-web` services |
...
Pubky Pulse uses email to send verification codes for passwordless authentication. You have two options:
